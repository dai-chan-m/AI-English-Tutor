...
// プロンプト生成関数
function generateTopicPrompt(level: string, existingTopics: string[]) {
  return `
...
function generateJapaneseExplanationPrompt(topic: string) {
  return `
...
function generateModelAnswerPrompt(topic: string, level: string) {
  return `
...
export async function GET(req: NextRequest) {
  try {
    ...
    const topicResponse = await openai.chat.completions.create({
      model: "gpt-4o",
      messages: [{ role: "user", content: generateTopicPrompt(level, existingTopics) }],
    });
    ...
    const japaneseExplanationResponse = await openai.chat.completions.create({
      model: "gpt-4o", 
      messages: [{ role: "user", content: generateJapaneseExplanationPrompt(topic) }],
    });
    ...
    const modelAnswerResponse = await openai.chat.completions.create({
      model: "gpt-4o",
      messages: [{ role: "user", content: generateModelAnswerPrompt(topic, level) }],
    });
